...
public class ClsOpenHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        sqLiteDatabase.execSQL("create table TblVehiculo(placa text primary key , marca text not null ,"+
                " modelo text not null, valor integer not null, activo text not null default 'si' )");

        sqLiteDatabase.execSQL("create table TblFactura(cod_factura text primary key, fecha text not null,"+
                " placa text not null, activo text not null default 'si',constraint pk_factura foreign key (placa) references TblVehiculo(placa) )");
    }
